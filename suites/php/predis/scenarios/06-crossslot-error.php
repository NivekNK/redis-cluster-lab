 ...
 * ¿Por qué Redis Cluster tiene esta limitación?
 * (Respuesta: Las operaciones atómicas no pueden cruzar nodos.
...
echo "Mensaje típico:\n";
echo "  CROSSSLOT Keys in request don't hash to the same slot\n\n";

// Test 1: Provocar CROSSSLOT con MGET
...
if ($slot1 !== $slot2) {
    ...
    try {
        ...
        if (strpos($e->getMessage(), 'CROSSSLOT') !== false || strpos($e->getMessage(), 'Cannot use') !== false) {
            error("¡CROSSSLOT ERROR (o bloqueado por Predis)!");
            info("Mensaje", $e->getMessage());
            
            echo "\n{$GREEN}✓ Esto es el comportamiento esperado{$NC}\n";
            echo "Predis intercepta operaciones multi-key entre slots y\n";
            echo "lanza una excepción antes de enviarlas a Redis para prevenir CROSSSLOT.\n";
        } else {
            echo "Error diferente: " . $e->getMessage() . "\n";
        }
    }
} else {
    echo "\n{$YELLOW}Las keys coinciden en el mismo slot por casualidad{$NC}\n";
    echo "Prueba con diferentes keys.\n";
}
...
echo "Los pipelines también requieren mismo slot:\n\n";
...
if ($slotA !== $slotB) {
    ...
    try {
        ...
        success("Pipeline funcionó (sorpresa!)");
    } catch (\Predis\Response\ServerException $e) {
        if (strpos($e->getMessage(), 'CROSSSLOT') !== false) {
            error("¡CROSSSLOT en pipeline!");
        } else {
...
if ($slotSource !== $slotDest) {
    ...
    try {
        $client->rpoplpush($source, $dest);
        success("RPOPLPUSH funcionó (sorpresa!)");
    } catch (\Exception $e) {
        if (strpos($e->getMessage(), 'CROSSSLOT') !== false || strpos($e->getMessage(), 'Cannot use') !== false) {
            error("¡CROSSSLOT ERROR (o bloqueado por Predis)!");
            echo "{$GREEN}✓ Comportamiento esperado: Predis intercepta operaciones multi-key entre slots.{$NC}\n";
        } else {
            echo "Error: " . $e->getMessage() . "\n";
        }
    }
...
// Test 7: Por qué existe CROSSSLOT
section("TEST 7: ¿Por qué existe CROSSSLOT?");
...
echo "✓ Redis Cluster no soporta operaciones atómicas cross-node\n";
...
echo "\n{$GREEN}Próximo escenario: make test php/predis/07-failover{$NC}\n";
